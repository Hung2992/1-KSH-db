...
- `GET /health`
- `GET /links`
- `GET /decisions?limit=10` (`x-role: analyst`)
- `GET /command-center/overview` (`x-role: analyst`)
- `GET /admin/roles` (`x-role: admin`)
- `GET /analytics/:module` (`x-role: analyst`)
- `GET /predict/:module` (`x-role: analyst`)
- `GET /commands?limit=20` (`x-role: operator`)
- `POST /commands/:id/execute` (`x-role: operator`)
- `GET /audit-logs?limit=20` (`x-role: admin`)
- `GET /deploy/public-config` (`x-role: admin`)
- `POST /security/drill/breach` (`x-role: operator`)
- `POST /events/:module`
- `POST /omega/evaluate`
- `POST /decisions/:id/approve` (`x-role: operator`)
...
- RBAC role contextet is tud állítani,
...
- `GET /command-center/overview` (top 10 critical event + modul státuszok)
- `GET /analytics/:module` (module KPI aggregate)
- `GET /predict/:module` (trend + next score becslés + javasolt action)
- `GET /commands` + `POST /commands/:id/execute` (automation pipeline queue)
...
Manual review flow-hoz `POST /decisions/:id/approve` endpoint is elérhető operator+ szerepkörtől.
