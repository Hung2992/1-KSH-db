...
test('prediction engine flags rising risk trend', () => {
  const prediction = forecastModuleRisk([20, 35, 55, 72]);
  assert.equal(prediction.trend, 'rising_risk');
  assert.ok(prediction.nextScore >= 70);
});

test('prediction action escalates security incident on high rising trend', () => {
  const prediction = forecastModuleRisk([25, 45, 68, 84]);
  const action = derivePredictionAction('security', prediction);
  assert.equal(action, 'escalate_incident');
});

test('automation policy maps finance block to hard commands', () => {
  const commands = deriveModuleCommands('finance', 'block');
  assert.deepEqual(commands, ['freeze_transaction', 'send_alert', 'audit_log']);
});
